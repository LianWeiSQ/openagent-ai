...
- current_slice: S6 completed - `ContextPackBuilder` 已用版本化 layered budget allocator 替换全局 pinned/priority greedy；hard reserve、soft quota、borrowing、recency/recoverability、anchor-kind 排序和 tool-exchange 原子组在 CLI/HTTP/replay/restart 使用同一策略。
- slice_boundary: `openagent.context_budget_policy.v1`、`openagent.context_budget_allocation.v1`、taxonomy/anchor-kind/scope/recency/recoverability 分类、required fitting 后 hard reserve、六类 soft quota 与共享 borrowing、assistant tool call/result 原子组、trace/receipt/public diagnostics、pack integrity、CLI/Bridge parity、HTTP provider/replay/restart、golden 与架构文档；不实现 S7 compact eval suite。
- success_criteria: latest user、goal/constraint 与其他 pinned 项进入 hard reserve，required overflow 可精确解释；最近会话优先于历史，同类按 scope/recoverability/priority/sequence 稳定排序；soft quota 未使用额度可借用；工具交互不被拆分；policy 随 replay spec 持久化，CLI/HTTP provider payload 与 restart/replay 选择一致；receipt/trace 不泄漏正文、凭据或 model option value；golden、全 workspace tests、strict clippy、format/diff/secret scan 通过；阶段提交已推送。
- milestones: S1-S6 completed; S7 pending.
- last_receipts:
...
- next_recommended_slice: S7 - compact eval/golden suite；默认下一轮只建立可重复长会话 corpus、语义保真/token savings/recovery parity 指标、基线与回归门禁，完成后独立推送。
- blockers: none
