...
pub fn route_unauthorized() -> HttpResponseSpec {
    ...
    headers.insert(
        ...
        Value::String(
            "Bearer realm=\"openagent-bridge\", Basic realm=\"openagent-bridge\"".to_string(),
        ),
    );
    HttpResponseSpec {
        status: 401,
    ...
    json!({
        ...
        "compatibility": {
            ...
            "sse": {
                ...
                "event_field": "method",
                "data": "JSON BridgeEvent envelope",
...
#[must_use]
pub fn emit_bridge_events(events: &[Value], output_format: &str, verbose: bool) -> CliRunResult {
...
pub fn parse_cli_args(args: &[String]) -> (HttpRuntimeConfig, bool, bool) {
    ...
    if let Some(path) = std::env::var_os("OPENAGENT_BRIDGE_AUTH_TOKEN_FILE") {
        config.auth_token = Some(auth_token_from_file(Path::new(&path)));
    }
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "--auth-token-file" => {
                if let Some(value) = args.get(index + 1) {
                    config.auth_token = Some(auth_token_from_file(Path::new(value)));
                    index += 1;
                }
            }
            "--username" | "-u" => {
...
fn auth_token_from_file(path: &Path) -> String {
    fs::read_to_string(path)
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
        .unwrap_or_else(|| "__openagent_invalid_auth_token_file__".to_string())
}
...
pub(super) fn handle_http_stream(
    ...
    write_http_response(stream, with_runtime_headers(response, config, &request))
}
...
pub(super) fn route_http_request(
    ...
    if !cors_request_allowed(request, config) {
...
pub(super) fn should_live_sse(request: &HttpRequest, config: &HttpRuntimeConfig) -> bool {
    if request.method != "GET"
        || !cors_request_allowed(request, config)
...
pub(super) fn write_live_sse_response(
    ...
    let cors_header = cors_response_origin(request, config)
...
pub(super) fn live_sse_events_after(
    ...
    let events = if let Some(turn_id) = turn_id {
        turn_bridge_events(config, turn_id)
    } else {
        all_bridge_events(config)
    };
    events
        .into_iter()
        .enumerate()
...
pub(super) fn with_runtime_headers(
    ...
    if let Some(origin) = cors_response_origin(request, config) {
        response
            .headers
            .insert("Access-Control-Allow-Origin".to_string(), json!(origin));
        response.headers.insert("Vary".to_string(), json!("Origin"));
    }
...
pub(super) fn cors_request_allowed(request: &HttpRequest, config: &HttpRuntimeConfig) -> bool {
    request
        .headers
        .get("origin")
        .is_none_or(|origin| cors_origin_configured(config, origin))
}

pub(super) fn cors_response_origin(
    request: &HttpRequest,
    config: &HttpRuntimeConfig,
) -> Option<String> {
    let origin = request.headers.get("origin")?.trim();
    cors_origin_configured(config, origin).then(|| origin.to_string())
}

fn cors_origin_configured(config: &HttpRuntimeConfig, origin: &str) -> bool {
    config
        .cors_origin
        .split(',')
        .map(str::trim)
        .filter(|configured| !configured.is_empty())
        .any(|configured| configured == "*" || configured.eq_ignore_ascii_case(origin))
}
