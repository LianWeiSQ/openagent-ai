...
- Skill CLI/API observability is now closed for Step10: CLI skills golden,
  ...
  shared usage/trace payload helpers for CLI and HTTP. The next runtime risk is
...
1. Shared SessionRunner facade: the first facade layer now owns shared
   ...
   terminal turn event envelopes plus usage/trace payload helpers for CLI and
   HTTP. Continue moving provider calls, task handoff, skill loading, pending
