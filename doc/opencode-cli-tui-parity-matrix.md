...
- Skill CLI/API observability is now closed for Step10: CLI skills golden,
  ...
  centralizes ToolContext construction plus question-answer JSON parsing for
  CLI and HTTP. The next runtime risk is the still-duplicated
  provider-step/tool-call/task loop, which should move behind the same facade.
...
| Shared runner contract | Shared profile schema, ToolContext construction, and question-answer parsing | `cargo test -p openagent-tools -q`; `cargo test -p openagent-cli binary_approval_and_question_responses_resume_paused_runs --test cli_commands -q`; `cargo check -p openagent-cli -p openagent-http-runtime` |
...
1. Shared SessionRunner facade: the first facade layer now owns shared
   ToolContext construction and question-answer parsing for CLI and HTTP.
   Continue moving provider calls, task handoff, skill loading, pending
   approval/question resume, and session events into the common runner until
   the duplicated loops disappear.
